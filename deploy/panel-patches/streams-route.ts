...
export async function GET(req: NextRequest) {
  ...
  if (withStats && streams.length) {
    ...
    const statsMap = await getStreamLiveStatsMap(ids);
  ...
  if (withStats && singleStatsId) {
    const statsMap = await getStreamLiveStatsMap([singleStatsId]);
    return NextResponse.json({ liveStats: statsMap.get(singleStatsId) ?? null });
  }

  if (paginate) {
    const total = await prisma.stream.count({ where });
    return NextResponse.json({ streams, total, page, pageSize });
  }
