...
echo "=== PM2: marketing on :3002 from $MARKETING ==="
if [ -f "$MARKETING/package.json" ]; then
  cd "$MARKETING"
  pm2 delete nexlify-web 2>/dev/null || true
  if [ ! -d .next ]; then PORT=3002 npm run build 2>/dev/null || npm run build || true; fi
  pm2 start npm --name nexlify-web -- start -- -H 127.0.0.1 -p 3002 2>/dev/null || \
    PORT=3002 HOSTNAME=127.0.0.1 pm2 start npm --name nexlify-web -- run start
  pm2 save
fi
