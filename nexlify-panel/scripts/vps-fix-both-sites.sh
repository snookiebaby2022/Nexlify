...
# - nexlify.live       → marketing site on 127.0.0.1:3002 (separate PM2 app)
...
for d in /home/nexlify /var/www/nexlify "$MARKETING_ROOT"; do
...
if command -v pm2 >/dev/null 2>&1; then
  chmod +x scripts/pm2-start.sh scripts/ensure-panel-env.sh scripts/verify-panel-upstream.sh
...
if [ -d "$MARKETING_ROOT" ] && command -v pm2 >/dev/null 2>&1; then
  (
    ...
    if [ ! -d .next ] && [ -f package.json ]; then
      ...
      PORT="${MARKETING_PORT}" npm run build 2>/dev/null || npm run build || true
    fi
    if pm2 describe "$MARKETING_PM2" >/dev/null 2>&1; then
      PORT="${MARKETING_PORT}" HOSTNAME=127.0.0.1 pm2 restart "$MARKETING_PM2" --update-env
    elif [ -f package.json ]; then
      PORT="${MARKETING_PORT}" HOSTNAME=127.0.0.1 pm2 start npm --name "$MARKETING_PM2" -- start -- -H 127.0.0.1 -p "${MARKETING_PORT}" 2>/dev/null || \
      PORT="${MARKETING_PORT}" HOSTNAME=127.0.0.1 pm2 start npm --name "$MARKETING_PM2" -- run start
      pm2 save
    fi
  )
else
...
sleep 4
ss -tlnp 2>/dev/null | grep -E ':3000|:3002' || true
if ./scripts/verify-panel-upstream.sh; then
  curl -s "http://127.0.0.1:${PANEL_PORT}/api/health"
  echo ""
else
...
curl -fsS -o /dev/null -w "marketing :${MARKETING_PORT} HTTP %{http_code}\n" "http://127.0.0.1:${MARKETING_PORT}/" 2>/dev/null || echo "marketing on :${MARKETING_PORT} not responding"
