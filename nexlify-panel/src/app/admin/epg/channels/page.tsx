...
export default function AdminEpgChannelsPage() {
  ...
  async function autoGenerateAll() {
    ...
    setBulkMsg(
      res.ok
        ? `Set channel_id on ${data.updated ?? 0} stream(s) that were empty.`
        : data.error ?? "Failed"
    );
    load();
  }
  ...
  return (
    <div className="space-y-6">
      ...
      <p className="text-sm" style={{ color: "var(--muted)" }}>
        <strong>channel_id</strong> is the client/Stalker ID (defaults to stream ID).
        <strong> epg_id</strong> is the XMLTV <code>channel=</code> value. Both are included in M3U and export files.
      </p>

      <DataTable
        headers={["Stream", "Category", "channel_id", "epg_id", ""]}
        rows={streams.map((s) => [
          s.name,
          s.category?.name ?? "—",
