...
import { resolveWebsiteHttpPort, resolvePanelListenPort } from "./server-ports";
...
export function xtreamUserInfo(line: LineWithBouquets, panelBaseUrl: string) {
  ...
  const websiteUrl = websiteBaseUrl(panelBaseUrl);
  let websitePort = resolveWebsiteHttpPort();
  try {
    const u = new URL(websiteUrl);
    if (u.port) websitePort = Number(u.port);
    else websitePort = u.protocol === "https:" ? 443 : 80;
  } catch {
    /* keep env default */
  }
  ...
  return {
    ...
    server_info: {
      ...
      https_port: String(websitePort),
      server_protocol: websiteUrl.startsWith("https") ? "https" : "http",
      rtmp_port: "0",
      timezone: "UTC",
