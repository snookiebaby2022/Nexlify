...
export async function activateLicenseKey(
  ...
  if (existing && existing.keyHash !== hash && existing.boundInstanceId === instanceId) {
    return { ok: false, error: "Another license is already active on this installation" };
  }
...
async function verifyOnlineIfRequired(
  ...
  if (!api) {
    if (process.env.NEXLIFY_LICENSE_REQUIRE_ONLINE === "1") {
      return {
        ok: false,
        error:
