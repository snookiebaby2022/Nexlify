...
if [ ! -f "$ROOT/$DIST/standalone/server.js" ] && [ "$DIST" != ".next" ] && [ -f "$ROOT/.next/standalone/server.js" ]; then
  echo "verify-standalone: $DIST/standalone missing — using .next"
  DIST=".next"
fi
...
CSS_DIR="$STANDALONE/.next/static/css"

CSS_COUNT="$(find "$CSS_DIR" -name '*.css' 2>/dev/null | wc -l | tr -d ' ')"

if [ "${CSS_COUNT:-0}" -lt 1 ]; then

  echo "ERROR: standalone has no CSS — run: bash scripts/prepare-standalone.sh"
...
echo "verify-standalone: OK ($COUNT chunk files, $CSS_COUNT CSS bundle(s))"
