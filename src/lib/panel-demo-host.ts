...
export function isPanelDemoHost(host: string): boolean {
  ...
  if (normalized === PANEL_DEMO_HOST) return true;
  const extras = process.env.PANEL_DEMO_HOSTS?.split(",") ?? [];
  return extras.some((e) => normalizeDomain(e) === normalized && Boolean(e.trim()));
}
