...
test("bufferingRisk warns before saturation", () => {
  assert.equal(bufferingRisk({ online: true, saturated: false, headroomPct: 15, loadPct: 80 }), "watch");
...
test("bufferingRisk is critical for offline or saturated delivery", () => {
  ...
  assert.equal(bufferingRisk({ online: true, saturated: true, headroomPct: 7, loadPct: 92, failedStreams: 1 }), "critical");
