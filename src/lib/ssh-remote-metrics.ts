...
const METRICS_SCRIPT = [
  ...
  '    oper=$(cat "/sys/class/net/$name/operstate" 2>/dev/null || true)',
  '    carrier=$(cat "/sys/class/net/$name/carrier" 2>/dev/null || true)',
  '    [ "$oper" = "up" ] || [ "$carrier" = "1" ] || continue',
  '    sp=$(cat "/sys/class/net/$name/speed" 2>/dev/null || echo 0); sp=${sp:-0}',
