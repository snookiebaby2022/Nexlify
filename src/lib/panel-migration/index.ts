...
export function previewMigrationBundle(bundle: MigrationBundle) {
  ...
  return {
    ...
    counts: {
      ...
      streamOptions: phase3?.streamOptions?.length ?? 0,
      streamArguments: phase3?.streamArguments?.length ?? 0,
      streamErrors: phase3?.streamErrors?.length ?? 0,
      extraBlobs: Object.keys(phase3?.extraTableBlobs ?? {}).length,
