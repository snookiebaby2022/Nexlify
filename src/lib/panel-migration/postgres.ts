...
import type { MigrationPhase2Data } from "./types";
...
> = {
  ...
  mag: {
    patterns: [/^mag_devices?$/i, /^mag$/i, /^stb_devices?$/i],
    penalty: /enigma|log/i,
  },
  enigma: {
    patterns: [/^enigma_devices?$/i, /^enigma$/i, /^enigma2_devices?$/i],
    penalty: /log/i,
  },
  bouquetStreams: {
    patterns: [
...
function enrichLinesFromJunction(
  lines: SqlTableData | null,
  junction: Record<string, unknown>[] | null,
  lineIdCol: string,
  bouquetIdCol: string
) {
  if (!lines || !junction?.length) return lines;
  const byLine = new Map<string, string[]>();
  for (const row of junction) {
    const lid = String(row[lineIdCol] ?? row.line_id ?? row.subscription_id ?? row.user_id ?? "");
    const bid = String(row[bouquetIdCol] ?? row.bouquet_id ?? row.package_id ?? "");
    if (!lid || !bid) continue;
    const list = byLine.get(lid) ?? [];
    list.push(bid);
    byLine.set(lid, list);
  }
  const idIdx = lines.columns.findIndex((c) => c === "id");
  let bouquetIdx = lines.columns.findIndex((c) =>
    ["bouquet", "bouquets", "bouquet_ids", "package_id", "packages"].includes(c)
  );
  if (bouquetIdx < 0) {
    lines.columns.push("bouquet");
    bouquetIdx = lines.columns.length - 1;
    for (const row of lines.rows) row.push(null);
  }
  for (const row of lines.rows) {
    const id = idIdx >= 0 ? String(row[idIdx]) : "";
    const extra = byLine.get(id);
    if (!extra?.length) continue;
    row[bouquetIdx] = JSON.stringify([...idsFromJunctionValue(row[bouquetIdx]), ...extra]);
  }
  return lines;
}

function detectLineJunctionCols(columns: string[]) {
  const lineCol =
...
export async function bundleFromPostgres(
  ...
  try {
    ...
    let lines = await load("lines");
    const lineJunctionRef = mapping.lineBouquets;
    if (lineJunctionRef) {
      const junctionRows = await fetchTable(client, lineJunctionRef.schema, lineJunctionRef.table);
      const { lineCol, bouquetCol } = detectLineJunctionCols(lineJunctionRef.columns);
      lines = enrichLinesFromJunction(lines, junctionRows, lineCol, bouquetCol);
    }
