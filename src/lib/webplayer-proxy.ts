...
/** Detect if a URL contains a non-standard port without an explicit protocol. */
export function hasNonStandardPort(raw: string): boolean {
  try {
...
export function assertAllowedPanelUrl(raw: string): URL {
  if (!raw.startsWith("http://") && !raw.startsWith("https://") && hasNonStandardPort(raw)) {
    throw new Error("Please include the protocol (http:// or https://) for panel URLs with a custom port.");
  }
