      ...
      for (let i = 0; i < bundle.providers.length; i++) {
        ...
        const created = await prisma.streamProvider.create({
          data: {
            ...
            maxStreams: p.maxStreams ?? null,
            notes: p.notes ?? null,
            remotePanelUrl: p.remotePanelUrl ?? null,
            remoteHost: p.remoteHost ?? null,
            remotePort: p.remotePort ?? null,
            remoteUsername: p.remoteUsername ?? null,
            remotePassword: p.remotePassword ?? null,
            remoteProtocol: p.remoteProtocol ?? null,
            remoteNotes: p.remoteNotes ?? null,
