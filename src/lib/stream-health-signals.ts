...
/** Viewer zap failures — never treat as origin dead. Never throw into live-auth/pulse. */
export async function markStreamViewerPlaybackFailed(streamId: string, detail: string): Promise<void> {
  try {
    ...
    console.warn("[stream-health] viewer fail stamp skipped", err instanceof Error ? err.message : err);
  }
}
...
export async function markStreamViewerPlaybackOk(streamId: string): Promise<void> {
  try {
    ...
    console.warn("[stream-health] viewer ok stamp skipped", err instanceof Error ? err.message : err);
  }
}
...
export async function markStreamSpliceOk(streamId: string): Promise<void> {
  try {
    ...
    console.warn("[stream-health] splice ok stamp skipped", err instanceof Error ? err.message : err);
  }
}
