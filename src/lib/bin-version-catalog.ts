...
export function catalogFfmpegVersions(binRoot?: string): BinVersionOption[] {
  ...
  const extras: [string, string][] = [
    ["80-cuda", `${ffmpegDir}/8.0_cuda/ffmpeg`],
    ["80-nvenc", `${ffmpegDir}/8.0_nvenc/ffmpeg`],
    ["80-optimized", `${ffmpegDir}/8.0/ffmpeg`],
    ["nvenc-80", `${ffmpegDir}/8.0_nvenc/ffmpeg`],
...
/** Nginx builds under /home/nexlify/bin/nginx */
export function catalogNginxVersions(binRoot?: string): BinVersionOption[] {
  const root = rootDir(binRoot);
  const nginxRoot = `${root}/nginx`;
  return [
    {
      id: "nginx-129-quic",
      label: "Nginx 1.29.0 (HTTP/2 + HTTP/3 + QUIC)",
      path: `${nginxRoot}/1.29.0/sbin/nginx`,
    },
    { id: "nginx-default", label: "Nginx bundled default", path: `${nginxRoot}/sbin/nginx` },
    { id: "nginx-system", label: "System /usr/sbin/nginx", path: "/usr/sbin/nginx" },
    { id: "nginx-rtmp", label: "Nginx RTMP module", path: `${root}/nginx_rtmp/sbin/nginx` },
  ];
}
