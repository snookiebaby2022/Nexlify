...
type RunCommandOptions = {
  stepName?: string;
  onStream?: (line: string) => void | Promise<void>;
};
...
function runCommand(
  ...
  return new Promise((resolve) => {
    ...
    let settled = false;
    let lineBuf = "";
    const flushLine = (chunk: string) => {
      lineBuf += chunk;
      const parts = lineBuf.split(/\r?\n/);
      lineBuf = parts.pop() ?? "";
      for (const line of parts) {
        if (options?.onStream) void options.onStream(line);
      }
    };
    const finish = (ok: boolean, text: string) => {
      ...
      settled = true;
      clearTimeout(timer);
      if (lineBuf.trim() && options?.onStream) void options.onStream(lineBuf);
...
export async function resolvePatchUpdateScript(repoPath: string): Promise<string | null> {
  const candidates = [
    ...
    path.join(repoPath, "scripts/apply-panel-fast-update.sh"),
...
async function resolvePanelRestartStep(repoPath: string): Promise<UpdateStep> {
  const safe = path.join(repoPath, "scripts/panel-restart-safe.sh");
  try {
    await access(safe);
    return {
...
async function tryRecoverPanel(repoPath: string, steps: PanelUpdateResult["steps"]) {
  ...
  const quick = await runCommand(repoPath, "bash", [script, "--quick"], { stepName: "recover panel" });
  steps.push({ name: "recover panel (rollback)", ok: quick.ok, output: quick.output });
  if (!quick.ok) {
    const full = await runCommand(repoPath, "bash", [script], { stepName: "recover panel" });
    steps.push({ name: "recover panel (rebuild)", ok: full.ok, output: full.output });
  }
}
...
async function patchUpdateSteps(patchScript: string, repoPath: string): Promise<UpdateStep[]> {
  ...
  return [
    ...
    { name: "prepare standalone", command: "bash", args: [patchScript, "build-standalone"] },
    await resolvePanelRestartStep(repoPath),
  ];
}
...
export async function runPanelUpdateWithProgress(
  ...
  /* Pre-update disk-space check */
  const diskCheck = await runCommand(repoPath, "bash", ["-c", "df -BG . | awk 'NR==2{print $4}' | tr -d 'G'"]);
  const freeGb = parseInt(diskCheck.output.trim(), 10);
  if (!Number.isNaN(freeGb) && freeGb < 2) {
    const msg = `Insufficient disk space (${freeGb}GB free, need 2GB+). Free up space before updating.`;
    steps.push({ name: "check disk space", ok: false, output: diskCheck.output });
  ...
  if (!ok) {
    ...
    const failed = steps.find((s) => !s.ok)?.name ?? steps[steps.length - 1]?.name ?? "unknown";
    const msg = `Update failed at step: ${failed}. Panel recovery was attempted automatically.`;
    await recordResult(settings, false, msg, fromVersion, fromVersion, "update", steps);
    return { ok: false, message: msg, steps, fromVersion, toVersion: fromVersion };
  }
