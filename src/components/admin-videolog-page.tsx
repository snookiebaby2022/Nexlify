...
export default function AdminVideoLogPage() {
  ...
  function load() {
    ...
    fetch(`/api/admin/streams?${params}`)
      .then((r) => r.json())
      .then((d) => {
        const rows = (d.streams ?? d.items ?? []) as StreamRow[];
        setStreams(rows);
        if (initialStreamId) {
          const match = rows.find((s) => s.id === initialStreamId);
          if (match) {
            setSelected(match);
            setType(match.type);
          }
  ...
  useEffect(() => {
    load();
    // eslint-disable-next-line react-hooks/exhaustive-deps -- reload when type changes; streamId handled on first load
  }, [type]);
  ...
  useEffect(() => {
    ...
    if (match) {
      setSelected(match);
      if (match.type !== type) setType(match.type);
    }
  }, [initialStreamId, streams, type]);
