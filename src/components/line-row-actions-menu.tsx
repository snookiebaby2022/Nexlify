...
export function LineRowActionsMenu({
  ...
  async function killConnections() {
    try {
      ...
      alert(e instanceof Error ? e.message : "Could not kill connections");
    }
  }
  ...
  async function remove() {
    ...
    try {
      ...
      alert(e instanceof Error ? e.message : "Could not delete line");
    }
  }
  ...
  const items: { label: string; icon: React.ReactNode; onClick: () => void; danger?: boolean }[] = [
    {
      ...
      icon: <Power size={15} />,
      onClick: () => void setStatus(line.status === "ACTIVE" ? "DISABLED" : "ACTIVE"),
    },
    {
      ...
      <>
        ...
        >
          ...
          <button
            type="button"
            role="menuitem"
            className="xui-lines-action-menu-item xui-lines-action-menu-item--download"
            onClick={() => {
              setDownloadModalOpen(true);
              onClose();
            }}
          >
            <span className="xui-lines-action-menu-icon">
              <Download size={15} />
            </span>
