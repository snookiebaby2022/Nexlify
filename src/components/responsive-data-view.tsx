...
export type ResponsiveDataViewProps = {
  ...
  /** Custom card renderer; falls back to label/value pairs. */
  renderCard?: (row: (string | React.ReactNode)[], index: number) => React.ReactNode;
  emptyMessage?: string;
  className?: string;
};
...
export function ResponsiveDataView({
  ...
  return (
    <div className={`responsive-data-view rounded-lg border ${className}`.trim()} style={{ borderColor: "var(--border)" }}>
      <div className="responsive-data-view__cards md:hidden divide-y" style={{ borderColor: "var(--border)" }}>
        {rows.length === 0 ? (
...
export function DataTable({
  ...
  return <ResponsiveDataView headers={headers} rows={rows} />;
}
