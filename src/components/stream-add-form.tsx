...
function LiveStreamForm({
  ...
  useEffect(() => {
    ...
    if (initial.streamUrl?.trim()) setSources([initial.streamUrl.trim()]);
  }, [initial]);
  ...
  useEffect(() => {
    ...
    fetch("/api/admin/bouquets")
      .then((r) => r.json())
      .then((d) =>
        setBouquetItems((d.bouquets ?? []).map((b: { id: string; name: string }) => ({ id: b.id, label: b.name })))
      );
    fetch("/api/admin/epg")
      .then((r) => r.json())
  ...
  async function create(e: React.FormEvent) {
    ...
    const res = await fetch("/api/admin/streams", {
      ...
      body: JSON.stringify({
        ...
        ...(extraSources.length > 0 ? { bitrates } : {}),
      }),
    });
    setSaving(false);
    if (!res.ok) {
      ...
      >
        <div className="grid lg:grid-cols-2 gap-5">
          <div className="space-y-4">
            <FormField label="Insert type">
              <select className={formSelectClass} style={formInputStyle} defaultValue="single" disabled>
        ...
        <Section title="Stream options">
          <div className="grid sm:grid-cols-2 lg:grid-cols-3 gap-5">
            ...
            />
            <YesNo
              label="Direct source (return direct URL, no restreaming)"
