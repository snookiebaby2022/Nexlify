...
type TreeGroup = { id: string; label: string; servers: StreamServer[] };
...
export function ServerTreePicker({
  ...
  const groups = useMemo((): TreeGroup[] => {
    ...
    const byProxy = new Map<string, StreamServer[]>();
    for (const s of servers) {
      if (!s.proxyId) continue;
      const key = s.proxyId;
      if (!byProxy.has(key)) byProxy.set(key, []);
      byProxy.get(key)!.push(s);
    }
    const out: TreeGroup[] = [];
    if (direct.length) out.push({ id: "__direct__", label: "Direct / main servers", servers: direct });
    for (const s of servers) {
      if (!s.proxy?.id || byProxy.get(s.proxy.id) === undefined) continue;
      if (out.some((g) => g.id === s.proxy!.id)) continue;
      out.push({
        id: s.proxy.id,
