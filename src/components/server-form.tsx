...
function uniquePortTags(...values: string[]): string[] {
  const out: string[] = [];
...
function parsePortTags(tags: string[], fallback: number): { primary: number; extras: number[] } {
  const nums = tags
    .map((t) => parseInt(t, 10))
    .filter((n) => Number.isFinite(n) && n > 0 && n <= 65535);
  if (!nums.length) return { primary: fallback, extras: [] };
  return { primary: nums[0], extras: nums.slice(1) };
}
...
export const defaultServerForm = (): ServerFormState => ({
  ...
  ...(() => {
    ...
    return {
      ...
      advHttpPorts: [String(STREAM_HTTP_PORT), "8080"],
      advHttpsPorts: [String(STREAM_HTTPS_PORT)],
...
function serverToForm(s: Record<string, unknown>): ServerFormState {
  ...
  return {
    ...
    advHttpsPorts: uniquePortTags(
      String(s.httpsPort ?? STREAM_HTTPS_PORT),
      ...(advanced.httpsPorts ?? []).map(String)
    ),
...
function buildPayload(form: ServerFormState, panelSettings: object | null) {
  ...
  const http = parsePortTags(form.advHttpPorts, form.port);
  const https = parsePortTags(form.advHttpsPorts, form.httpsPort);
  return {
    name: form.name,
    host: form.host,
...
export function ServerForm({
  ...
  useEffect(() => {
    fetch("/api/admin/servers/interfaces")
      .then((r) => r.json())
      .then((d) => {
        const list: { name: string; servers: string[] }[] = d.interfaces ?? [];
        setKnownInterfaces(list);
        if (mode === "create") {
          setForm((f) => {
            if (f.netInterface.trim()) return f;
            const fromFleet = list.find((i) => i.servers.length > 0) ?? list[0];
            return fromFleet ? { ...f, netInterface: fromFleet.name } : f;
          });
        }
      })
      .catch(() => {});
  ...
  async function save(e: React.FormEvent) {
    ...
    const panelSettings = buildServerPanelSettingsJson(
      existingPanelSettings,
      {
        ...
        advanced: {
          ...
          httpPorts: parsePortTags(form.advHttpPorts, form.port).extras,
          httpsPorts: parsePortTags(form.advHttpsPorts, form.httpsPort).extras,
          geoIpPriority: form.advGeoIpPriority,
        },
        ssl: {
    ...
    if (mode === "create") {
      ...
      if (data.portSync && !data.portSync.ok) {
        alert(`Created, but port sync failed: ${data.portSync.message}`);
      }
        ...
        <div className="p-4 sm:p-6 md:p-8 space-y-4">
          {tab === "details" && (
            <div className="space-y-1 max-w-2xl">
              ...
              <p className="text-xs pt-2" style={{ color: "var(--muted)" }}>
                Stream edge HTTP: <strong>{STREAM_HTTP_PORT}</strong> · Panel:{" "}
            ...
            <div className="space-y-4 max-w-2xl">
              ...
              <FormField label="Primary interface">
                <input
                  ...
                  placeholder="eth0"
                  list="server-net-interfaces"
                />
                <datalist id="server-net-interfaces">
                  {knownInterfaces.map((i) => (
                    <option
                      key={i.name}
                      value={i.name}
                      label={i.servers.length ? `Used on ${i.servers.join(", ")}` : "Common default"}
                    />
                  ))}
                </datalist>
                <p className="text-xs mt-1" style={{ color: "var(--muted)" }}>
                  Suggestions from your existing servers and load balancers. Pick the NIC the stream agent should bind to.
                </p>
              </FormField>
          ...
          {tab === "advanced" && (
            <div className="space-y-5 max-w-3xl">
              <FormField label="HTTP Ports">
                <TagInput
                  tags={form.advHttpPorts}
                  onChange={(advHttpPorts) => setForm({ ...form, advHttpPorts })}
                  placeholder="80"
                  inputMode="numeric"
                />
                <InfoTip title="Ports the stream server listens on for HTTP (Xtream, HLS). First tag is the primary port." />
              </FormField>
              <FormField label="HTTPS Ports">
                <TagInput
                  tags={form.advHttpsPorts}
                  onChange={(advHttpsPorts) => setForm({ ...form, advHttpsPorts })}
                  placeholder="443"
                  inputMode="numeric"
                />
                <InfoTip title="HTTPS / SSL ports for secure playback and player_api." />
              </FormField>
              <FormField label="RTMP Port">
                ...
                />
                <InfoTip title="RTMP ingest port (0 or empty = disabled)." />
              </FormField>
              <Toggle
                label="Disable Ramdisk"
                checked={form.advDisableDiskRam}
                onChange={(advDisableDiskRam) => setForm({ ...form, advDisableDiskRam })}
              />
              <FormField label="Network Interface">
                <select
                  className={formSelectClass}
                  style={formInputStyle}
                  value={form.netInterface || "auto"}
                  onChange={(e) => setForm({ ...form, netInterface: e.target.value === "auto" ? "" : e.target.value })}
                >
                  <option value="auto">auto</option>
                  {knownInterfaces.map((i) => (
                    <option key={i.name} value={i.name}>
                      {i.name}
                      {i.servers.length ? ` (${i.servers.join(", ")})` : ""}
                    </option>
                  ))}
                </select>
                <InfoTip title="Primary network interface for outbound streams on this server." />
              </FormField>
              <FormField label="Network Speed - Mbps">
                <input
                  type="number"
                  className={`${formInputClass} max-w-[200px]`}
                  style={formInputStyle}
                  ...
                  placeholder="1000"
                />
                <InfoTip title="Advertised link speed for load balancing (0 = unlimited)." />
              </FormField>
              <FormField label="GeoIP Priority">
                <select
                  className={formSelectClass}
                  style={formInputStyle}
                  value={form.advGeoIpPriority}
                  onChange={(e) =>
                    setForm({
                      ...form,
                      advGeoIpPriority: e.target.value as "low" | "medium" | "high",
                    })
                  }
                >
                ...
                <InfoTip title="Geo routing weight when multiple servers match a viewer region." />
              </FormField>
              <details className="rounded border px-4 py-3" style={{ borderColor: "var(--border)" }}>
                <summary className="cursor-pointer text-sm font-medium">More advanced options</summary>
                <div className="mt-4 space-y-4 pt-2 border-t" style={{ borderColor: "var(--border)" }}>
