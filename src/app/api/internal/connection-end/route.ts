...
/** Edge proxy calls this when the viewer disconnects so Live Connections clears promptly. */
export async function POST(req: NextRequest) {
  if (!isAuthorizedInternalRequest(req)) {
    return new NextResponse(null, { status: 403 });
  }

  let body: { lineId?: string; streamId?: string; ip?: string };
  try {
  ...
  await removeConnection(lineId, streamId, body.ip ?? "");
