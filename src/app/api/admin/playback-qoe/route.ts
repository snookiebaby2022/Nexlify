...
export async function GET(req: NextRequest) {
  ...
  const saturated = online.filter((s) => s.saturated);
  const worst = online.slice().sort((a, b) => a.headroomPct - b.headroomPct)[0];

  return NextResponse.json({
    liveConnections: rows.length,
    stallSessions,
    avgFirstPictureMs: ttfpN ? Math.round(ttfpSum / ttfpN) : null,
    servers: online.length,
    saturatedServers: saturated.length,
    worstHeadroomPct: worst?.headroomPct ?? 100,
    worstServerName: worst?.server.name ?? null,
    capMbps: online.reduce((n, s) => n + s.capMbps, 0),
    usedMbps: Math.round(online.reduce((n, s) => n + s.bandwidthMbps, 0) * 10) / 10,
  });
}
