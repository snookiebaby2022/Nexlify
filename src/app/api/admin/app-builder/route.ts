...
import {
  buildBrandedAndroidProject,
  writeBrandedAppConfig,
} from "@/lib/app-builder-apk";
...
export async function POST(req: Request) {
  ...
  // Produce branded config + Android project (APK when NEXLIFY_APK_BUILD=1).
  try {
    const buildInput = {
      buildId: build.id,
      appName,
    ...
    const { downloadUrl } = await writeBrandedAppConfig(buildInput);
    const android = await buildBrandedAndroidProject(buildInput);
    const completed = await prisma.appBuild.update({
      where: { id: build.id },
      data: {
        status: android.apkUrl ? "COMPLETED" : "BUILD_READY",
        downloadUrl: android.apkUrl ?? downloadUrl,
        completedAt: new Date(),
        config: {
          ...(config as object),
          androidProject: android.projectZipUrl,
          buildLog: android.buildLog,
        } as Prisma.InputJsonValue,
      },
    });
    return NextResponse.json({
      build: completed,
      configUrl: downloadUrl,
      androidProjectUrl: android.projectZipUrl,
      apkUrl: android.apkUrl,
      message: android.apkUrl
        ? "Branded APK ready for download."
        : "Android project prepared. Set NEXLIFY_APK_BUILD=1 on the server to compile APK automatically.",
    });
  } catch {
    return NextResponse.json({
      build,
      message: "Build queued; branded package write failed — retry or use external pipeline.",
    });
  }
  } catch (e) {
