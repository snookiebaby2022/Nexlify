...
export function AdminBouquetsClient({
  initialBouquets = [],
}: {
  initialBouquets?: BouquetListItem[];
}) {
  const [bouquets, setBouquets] = useState<BouquetListItem[]>(initialBouquets);
  const [error, setError] = useState<string | null>(null);
  const [loading, setLoading] = useState(!initialBouquets.length);
  ...
  useEffect(() => {
    if (initialBouquets.length) return;
    load();
  }, [load, initialBouquets.length]);
