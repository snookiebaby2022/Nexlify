  ...
  return (
    <form onSubmit={save} className="space-y-6 pb-8">
      ...
      <SettingsPanel title="Export format">
        <label className="block text-sm">
          <span style={{ color: "var(--muted)" }}>Manual / scheduled export format</span>
          <select
            className={inputClass}
      ...
      <SettingsPanel
        title="S3 placeholder (roadmap)"
        info="Optional env for future automated S3 upload. Leave blank until wired on your VPS."
      >
        <div className="grid md:grid-cols-2 gap-4 w-full">
          <label className="block text-sm">
            <span style={{ color: "var(--muted)" }}>S3 bucket</span>
            <input
              className={inputClass}
              style={inputStyle}
              placeholder="nexlify-backups"
              value={data.s3Bucket ?? ""}
              onChange={(e) => setData({ ...data, s3Bucket: e.target.value })}
            />
          </label>
          <label className="block text-sm">
            <span style={{ color: "var(--muted)" }}>S3 region</span>
            <input
              className={inputClass}
              style={inputStyle}
              placeholder="eu-west-1"
              value={data.s3Region ?? ""}
              onChange={(e) => setData({ ...data, s3Region: e.target.value })}
            />
          </label>
        </div>
      </SettingsPanel>
      ...
      <SettingsSaveBar saving={saving} msg={msg}>
        ...
        >
          {running ? "Running…" : "Run full backup now"}
        </button>
      </SettingsSaveBar>
