...
if curl -sf "http://127.0.0.1:${PORT}/pricing" >/tmp/nx-price.html 2>/dev/null; then
  ...
  if grep -q 'Nexlify License' /tmp/nx-price.html && ! grep -q 'Top Tier' /tmp/nx-price.html; then
...
echo "  2. npx tsx scripts/sync-plans-vps.ts               # single £50 plan in DB"
echo "  3. npm run build && pm2 restart nexlify-web        # after uploading new src"
echo ""
