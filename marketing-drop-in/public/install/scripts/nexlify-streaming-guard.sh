...
nexlify_panel_process_up() {
  if ! command -v pm2 >/dev/null 2>&1; then
    return 1
  fi
  local n
  n="$(pm2 jlist 2>/dev/null | node -e "
    try {
...
nexlify_refuse_restart_if_streaming_busy() {
  ...
  if ! nexlify_panel_process_up; then
    echo "[streaming-guard] panel process is down — allow restart (502 recovery)"
    return 0
  fi
