...
server {
    ...
    # MAG/StbEmu often omit the trailing slash; nginx 301 /c → /c/ = black screen.
    location = /c {
...
# Remote-splice: refuse media on :443 (same as :80 lock).
# shellcheck disable=SC1091
if [ -f "$ROOT/scripts/playback-topology.sh" ]; then
...
echo "[panel-https] OK ${DOMAIN} :443 → panel upstream (media refused when remote-splice)"
