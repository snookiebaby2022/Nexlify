#!/usr/bin/env bash
# Rematch PANEL_INTERNAL_SECRET on STREAM NODES only.
# On the customer panel (45.88.138.18 / live-routing.lock) this script exits
# immediately and stops nexlify-iptv-edge — nginx owns :8080.
set -euo pipefail
...
if nexlify_panel_must_not_run_iptv_edge; then
  echo "[rematch-iptv-edge] SKIP: this panel must not run nexlify-iptv-edge (nginx owns live :8080)"
