...
nexlify_capacity_compute() {
  ...
  # CPU ceiling: warm shared-HLS remux still costs CPU. Oversizing FPM past this
  # admits more sessions than the box can feed → buffering long before "max".
  # ~20 concurrent MPEG-TS readers per core is a safe colocated floor.
  local cpu_cap=$((nproc * 20))
  [ "$cpu_cap" -lt 32 ] && cpu_cap=32
  if [ "$role" = "colocated" ]; then
    # Panel shares the box — leave headroom for Next/Postgres.
    cpu_cap=$((nproc * 15))
    [ "$cpu_cap" -lt 32 ] && cpu_cap=32
  fi
  if [ "$media" -gt "$cpu_cap" ]; then
    media="$cpu_cap"
  fi
