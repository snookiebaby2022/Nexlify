      ...
      <div>
        ...
        <Step number={2} title="Run the installer">
          <p>
            ...
            <code className="text-violet-300">YOUR_SERVER_IP</code> with your VPS IP address — only{" "}
            <code className="text-violet-300">--domain</code> is required. You can add a custom domain later
            inside the panel under <strong className="text-white">Admin → Settings → Domains & SSL</strong>.
          </p>
          ...
          <p className="text-xs text-slate-500">
            ...
            <code className="text-violet-300">http://YOUR_SERVER_IP/login</code>, never{" "}
        ...
        <Step number={3} title="Sign in">
          ...
          <ul className="list-disc list-inside space-y-1">
            <li>
              ...
              <code className="text-violet-300">http://YOUR_SERVER_IP/login</code> (no port in the address bar)
            </li>
            <li>
        ...
        <Step number={5} title="Optional: add a domain + HTTPS later">
          <p>
            Once the panel is running, point your domain A record at your VPS, then add it inside the panel under{" "}
            <strong className="text-white">Admin → Settings → Domains & SSL</strong>. If you prefer to set it
            up during install instead, use:
          </p>
          <CodeBlock command={domainCommand}>
            <span className="text-slate-500">$</span> {domainCommand}
          </CodeBlock>
          <p className="text-xs text-slate-500">DNS must point at your VPS before running with --email.</p>
        </Step>
          ...
          <table className="w-full text-sm">
            ...
            <tbody className="text-[var(--muted)]">
              {[
                ...
                ["--skip-upgrade", "Do not run apt-get upgrade before install", "No"],
              ].map(([flag, desc, req]) => (
        ...
        <div className="flex items-start gap-3">
          ...
          <div>
            ...
            <p className="text-sm text-amber-200/70 mt-1">
              ...
              . The panel should always open at <code className="text-amber-200">http://YOUR_SERVER_IP/login</code> with no
              port number.
            </p>
          </div>
        </div>
