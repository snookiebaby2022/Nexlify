...
export const INSTALLER_VERSION = "v190";

export const cleanReinstallCommand = `sudo rm -rf ${PANEL_INSTALL_DIR}`;

export const simpleInstallCommand = `curl -fsSL 'https://nexlify.live/install/panel.sh?${INSTALLER_VERSION}' | sudo bash -s -- --domain YOUR_SERVER_IP`;

export function buildOneClickInstallCommand(opts: {
  domain?: string;
  email?: string;
}): string {
  const domain = opts.domain ?? "YOUR_SERVER_IP";
...
export const cleanReinstallWithFreshFlag = `curl -fsSL 'https://nexlify.live/install/panel.sh?${INSTALLER_VERSION}' | sudo bash -s -- --fresh --domain YOUR_SERVER_IP`;

export const wgetInstallExample = `wget -qO- 'https://nexlify.live/install/panel.sh?${INSTALLER_VERSION}' | sudo bash -s -- --domain YOUR_SERVER_IP`;
...
export const credentialsHelp = {
  ...
  fields: [
    { key: "login_url", label: "Open in browser — no port number needed (http://YOUR_SERVER_IP/login)" },
