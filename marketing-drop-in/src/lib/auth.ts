...
export async function getSessionUser(): Promise<SessionUser | null> {
  try {
    ...
    const isStaticGen =
      ...
      (caught.message.includes("couldn't be rendered statically because it used `cookies`") ||
        (caught as { digest?: string }).digest === "DYNAMIC_SERVER_USAGE");
    if (!isStaticGen) {
      console.error("[auth] getSessionUser failed (database or session error):", caught);
    }
    return null;
  }
}
