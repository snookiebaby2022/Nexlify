...
for root in roots:
    ...
    for path in root.glob("*.conf"):
        try:
            ...
            continue
        if "proxy_pass http://nexlify_panel" not in text and "proxy_pass http://nexlify_panel_backend" not in text:
            continue
        # Never attach the hold page to a vhost whose only job is MPEG-TS /live/.
        if "location ~ ^/(live|timeshift|movie|series)/" in text and "proxy_pass http://nexlify_panel" not in text:
            # 8080 live vhost may still have location / → panel backend; hold there is OK.
            pass
        orig = text
