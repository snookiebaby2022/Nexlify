...
echo "Removing marketing-only drop-ins (not part of panel build)..."
rm -rf marketing-drop-in promo-for-nexlify-web 2>/dev/null || true
...
if command -v pm2 >/dev/null 2>&1; then
  ...
  echo ""
  echo "Syncing IPTV ports, nginx stream edge, and firewall…"
  bash scripts/sync-panel-ports.sh 2>/dev/null || echo "WARN: run sudo bash scripts/sync-panel-ports.sh on the server"
  echo ""
