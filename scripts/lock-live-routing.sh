...
# Classic LB (nginx/PHP+FFmpeg): panel still 502s /live; media is never Node splice on Main.
# Usage:
...
[ -z "$TOPO" ] && TOPO="${NEXLIFY_PLAYBACK_TOPOLOGY:-remote-splice}"
EDGE_KIND="node-splice"
if [ "$TOPO" = "classic-lb" ] || [ "${NEXLIFY_CLASSIC_LB:-0}" = "1" ]; then
  EDGE_KIND="classic-ffmpeg"
  TOPO="classic-lb"
fi
...
edge=classic-lb: nginx/PHP+FFmpeg restream on stream nodes; legacy: Node splice. Panel must not run nexlify-iptv-edge.
...
if [ -f "$EDGE_FILE" ] && [ "$EDGE_KIND" != "classic-ffmpeg" ]; then
  chattr +i "$EDGE_FILE"
...
echo "LIVE_ROUTING_LOCKED topology=${TOPO} edge_kind=${EDGE_KIND}"
