 ...
 *
 * Usage (on panel host):
 *   node scripts/restore-xui-crypt-passwords.cjs /tmp/xui-users-passwords.json
 */
const fs = require("fs");
...
async function main() {
  ...
  for (const u of users) {
    ...
    const crypt = map[u.username] || lowerMap[u.username.toLowerCase()];
    ...
    } else if (u.passwordHash === crypt || u.passwordHash.startsWith("$6$")) {
      skipped++;
    } else {
      await prisma.panelUser.update({
        where: { id: u.id },
