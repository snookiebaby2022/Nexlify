...
STREAMING="${NEXLIFY_STREAMING_OPTIMIZED:-0}"
TARGET="${PANEL_INSTANCES:-4}"
SPARE="${NEXLIFY_PANEL_WORKER_SPARE:-1}"
MAX="${NEXLIFY_PANEL_INSTANCES_MAX:-6}"
if [ "$STREAMING" = "1" ]; then
  # XUI-style: panel is API-only; extra workers OOM and starve Redis without helping IPTV.
  TARGET="${PANEL_INSTANCES:-2}"
  SPARE=0
  MAX="${NEXLIFY_PANEL_INSTANCES_MAX:-$TARGET}"
  if [ "$MAX" -gt "$TARGET" ]; then MAX="$TARGET"; fi
  want="$TARGET"
else
...
if [ "$current" -gt "$want" ]; then
  echo "[scale-live] capping nexlify ${current} → ${want} (streaming profile)"
